...
mod instance_type_tests {
    ...
    fn test_cost_estimation_known_types() {
        ...
        for (instance_type, expected_cost) in test_cases {
            ...
            assert_eq!(cost, expected_cost, "Cost for {} should be {}", instance_type, expected_cost);
        }
    }
...
mod tag_tests {
    ...
    fn test_project_tag_filtering() {
        ...
        // Filter by project
        let matches_project = tags.iter()
            .any(|(k, v)| k == "trainctl:project" && v == "project-a");
        assert!(matches_project);
        
        let matches_wrong_project = tags.iter()
    ...
    fn test_user_tag_filtering() {
        ...
        // Filter by user
        let matches_user = tags.iter()
            .any(|(k, v)| k == "trainctl:user" && v == "alice");
        assert!(matches_user);
        
        let matches_wrong_user = tags.iter()
            .any(|(k, v)| k == "trainctl:user" && v == "bob");
...
mod az_tests {
    ...
    fn test_az_format_validation() {
        ...
        for az in valid_azs {
            ...
            assert!(last_char.is_ascii_lowercase(), 
                "AZ {} last character should be lowercase letter, got: {}", az, last_char);
        }
    }
