...
use trainctl::error::{Result, TrainctlError, IsRetryable};
...
async fn test_retry_backoff_timing() {
    ...
    let _result = policy.execute_with_retry(|| async {
        let count = call_count.fetch_add(1, Ordering::SeqCst);
        if count < 2 {
...
fn test_is_retryable_trait() {
    ...
    let io_error = TrainctlError::Io(std::io::Error::new(
        std::io::ErrorKind::Other,
        "test"
    ));
    ...
    let config_error = TrainctlError::Config(
        trainctl::error::ConfigError::InvalidProvider("test".to_string())
    );
