...
/// Helper to check if file exists on instance via SSM
async fn file_exists_on_instance(
    ssm_client: &SsmClient,
    instance_id: &str,
    path: &str,
) -> Result<bool, Box<dyn std::error::Error>> {
    let cmd = format!("test -f {} && echo 'EXISTS' || echo 'NOT_FOUND'", path);
...
async fn test_full_training_workflow() {
    ...
    let dir_exists = file_exists_on_instance(&ssm_client, &instance_id, &format!("{}/.", project_dir))
        .await
        .unwrap_or(false);
