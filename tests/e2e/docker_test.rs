...
async fn test_docker_training() {
    ...
    if !dockerfile.exists() {
        eprintln!("Skipping Docker test: Dockerfile not found at {:?}", dockerfile);
        return;
    }
    ...
    let instance_output = std::process::Command::new("cargo")
        .args(["run", "--release", "--", "aws", "create", "t3.medium", "--spot"])
        .output()
    ...
    let train_output = std::process::Command::new("cargo")
        .args([
            ...
            if PathBuf::from("training/train_mnist_e2e.py").exists() {
...
"#;
    ...
    let _ = std::process::Command::new("cargo")
        .args(["run", "--release", "--", "aws", "terminate", &instance_id, "--force"])
        .output();
}
