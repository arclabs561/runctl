...
async fn test_config_init_workflow() {
    ...
    let output = run_runctl_command(&["init", "--output", config_path.to_str().unwrap()], None)
    ...
    // Test: Show config
    let output = run_runctl_command(&["config", "show"], None).expect("Failed to run config show");
...
async fn test_config_validation() {
    ...
    let output =
        run_runctl_command(&["config", "validate"], None).expect("Failed to run config validate");
...
async fn test_aws_create_dry_run() {
    ...
    let output = run_runctl_command(&["aws", "create", "t3.micro", "--dry-run"], None)
...
async fn test_resources_list() {
    ...
    // Test: List resources
    let output =
        run_runctl_command(&["resources", "list"], None).expect("Failed to run resources list");
    ...
    assert!(
        output.status.success() || stderr.contains("AWS") || stderr.contains("credentials"),
        "Command should succeed or fail with AWS-related error\nStdout: {}\nStderr: {}",
        stdout,
        stderr
    );
...
async fn test_status_command() {
    ...
    // Test: Status command
    let output = run_runctl_command(&["status"], None).expect("Failed to run status command");
...
async fn test_error_handling() {
    ...
    let output = run_runctl_command(&["aws", "terminate", "invalid-instance-id"], None)
    ...
    let output = run_runctl_command(&["s3", "list", "invalid-path"], None)
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(json_str) {
            ...
            assert!(
                json.get("error").is_some()
                    || json.get("success") == Some(&serde_json::json!(false)),
...
async fn test_complete_workflow() {
    ...
    // Step 4: Check status
    let output = run_runctl_command(&["status"], None).expect("Failed to check status");
    assert_command_success(&output, &["status"]);
}
