...
use std::env;
use trainctl::safe_cleanup::{CleanupSafety, safe_cleanup, CleanupResult};
...
async fn test_protected_resources() {
    ...
    let can_delete = safety.can_delete(&protected_resource.id, &tracker, Some(tracked.created_at), false).await.unwrap();
