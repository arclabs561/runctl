...
EBS (Elastic Block Store) volume support has been added to `runctl` to enable persistent storage for training workloads, especially valuable for spot instances.

## Commands

EBS commands are available under `runctl aws ebs`:
...
# List volumes
runctl aws ebs list
runctl aws ebs list --detailed

# Attach volume to instance
runctl aws ebs attach vol-xxxxx --instance-id i-xxxxx --device /dev/sdf

# Detach volume
runctl aws ebs detach vol-xxxxx

# Delete volume
runctl aws ebs delete vol-xxxxx --force

# Create snapshot
runctl aws ebs snapshot vol-xxxxx --description "Checkpoint backup" --name backup-2025-01-15

# List snapshots
runctl aws ebs snapshot-list
runctl aws ebs snapshot-list --volume-id vol-xxxxx --detailed

# Restore volume from snapshot
runctl aws ebs restore snap-xxxxx --size 100 --type gp3 --name restored-volume

# Pre-warm volume (populate from S3)
runctl aws ebs pre-warm vol-xxxxx s3://bucket/data/ --mount-point /mnt/data
```
...
runctl aws create --spot --ebs-volume vol-xxxxx
```

### 2. Checkpoint Persistence
```bash
# Create checkpoint volume
runctl aws ebs create --size 100 --type gp3 --name checkpoints

# Attach to training instance
runctl aws ebs attach vol-xxxxx --instance-id i-xxxxx --device /dev/sdf

# Create snapshot backup
runctl aws ebs snapshot vol-xxxxx --description "Daily checkpoint backup"
```
...
### Tagging
- Automatic `CreatedBy: runctl` tag
...
2. **Instance integration**: Add `--ebs-volume` flag to `runctl aws create`
3. **Auto-mount**: Generate user-data script to auto-mount volumes
4. **Volume monitoring**: Add volume status to `runctl resources list`
...
```bash
# Create test volume
runctl aws ebs create --size 10 --type gp3 --name test-volume

# Verify creation
runctl aws ebs list --name test-volume
