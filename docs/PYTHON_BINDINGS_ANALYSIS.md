...
**Question**: Should we expose Python bindings for runctl?

## Current State

### What Exists
- ✅ Rust CLI tool (`runctl` binary)
...
- Python scripts can call `runctl` via subprocess
...
1. **Orchestration from Training Scripts**
   ...
   instance = runctl.aws.create_instance(instance_type="g4dn.xlarge")
   runctl.aws.train(instance, script="train.py", sync_code=True)
   ```

2. **Integration with ML Frameworks**
   ```python
   # In PyTorch Lightning callback
   from runctl import checkpoint
...
3. **Workflow Automation**
   ...
   for config in configs:
       instance = runctl.create_instance(config)
       runctl.train(instance, config.script)
       runctl.monitor(instance)
   ```
...
3. **Limited Value**
   - Most runctl operations are "fire and forget"
...
5. **Current Architecture**
   - runctl is CLI-first, not library-first
...
3. **Focus on core value**: runctl's value is in CLI orchestration, not programmatic API
...
3. **Library-first use cases**: Users want to build on runctl as a library
...
1. **Better JSON output**
   ```bash
   runctl aws create --output json | jq -r '.instance_id'
   ```

2. **Python helper scripts**
   ```python
   # scripts/runctl_wrapper.py
   import subprocess
   import json
   
   def create_instance(instance_type):
       result = subprocess.run(
           ["runctl", "aws", "create", "--instance-type", instance_type, "--output", "json"],
...
3. **uvx scripts**
   ...
   import runctl  # Would be a thin wrapper around CLI
   ```
...
```
runctl/
├── runctl-core/     # Pure Rust library (current src/)
├── runctl-cli/      # CLI binary (current main.rs)
└── runctl-py/       # PyO3 bindings (new)
```

### Steps
1. Refactor to workspace structure
2. Add `runctl-py/` crate with PyO3
3. Expose key functions: `create_instance`, `train`, `monitor`, `list_resources`
4. Use `maturin` for packaging
5. Publish to PyPI as `runctl` package
