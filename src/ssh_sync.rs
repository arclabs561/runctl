...
pub async fn sync_code_native(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        sess.userauth_pubkey_file(&user, None, Path::new(&key_path), None)
            .map_err(|e| TrainctlError::Ssm(format!("SSH authentication failed: {}. Check key permissions (chmod 600 {})", e, key_path)))?;
...
fn sync_incremental_blocking(
    ...
    let exclusions = [
        ".git", "checkpoints", "results", "data", "__pycache__", 
        "*.pyc", ".aim", "node_modules", ".venv"
    ];
    ...
    for file_path in &files_to_sync {
        ...
        let remote_path_parent = Path::new(&remote_path).parent()
            .ok_or_else(|| TrainctlError::Io(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "Invalid remote path"
            )))?;
        ...
        let mut remote_file = sftp.create(Path::new(&remote_path))
            .map_err(|e| TrainctlError::Ssm(format!("Failed to create remote file {}: {}", remote_path, e)))?;
        
        remote_file.write_all(&contents)
            .map_err(|e| TrainctlError::Ssm(format!("Failed to write to {}: {}", remote_path, e)))?;
        ...
        synced += 1;
        if let Some(ref p) = pb {
            p.set_message(format!("Synced {}/{} files...", synced, files_to_sync.len()));
        }
    }
...
fn sync_full_tar_blocking(
    ...
    let mut archive_data = Vec::new();
    {
        ...
        let exclusions = [
            ".git", "checkpoints", "results", "data", "__pycache__", 
            ".aim", "node_modules", ".venv"
        ];

        for entry in WalkDir::new(project_root).into_iter() {
            let entry = entry.map_err(|e| TrainctlError::Io(std::io::Error::new(
    ...
    info!("Full sync completed: {} bytes transferred", archive_data.len());
    Ok(())
}
