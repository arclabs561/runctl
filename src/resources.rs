...
pub async fn handle_command(cmd: ResourceCommands, config: &Config, output_format: &str) -> Result<()> {
    match cmd {
        ...
        ResourceCommands::StopAll { dry_run, force, platform } => {
...
async fn list_aws_instances(
    ...
    for inst in &filtered_instances {
        grouped.entry(inst.instance_type.clone()).or_default().push(inst);
    }
            ...
            if detailed {
                ...
                if !inst.tags.is_empty() {
                    let key_tags: Vec<String> = inst.tags.iter()
                        .filter(|(k, _)| k == "Name" || k == "trainctl:project" || k == "trainctl:created" || k == "CreatedBy")
                        .take(3)
                        .map(|(k, v)| {
                            // Clean up tag keys for display
                            let display_key = if k == "trainctl:project" { "project" } 
                                else if k == "trainctl:created" { "created" }
                                else { k };
...
async fn show_summary(_config: &Config, output_format: &str) -> Result<()> {
    ...
    if summary.total_cost_estimate > hourly_threshold {
        ...
        println!("{} {}",
            ...
            style(format!("Hourly cost (${:.2}/hr) exceeds threshold (${}/hr)", 
                summary.total_cost_estimate, hourly_threshold)).red().bold()
        );
        ...
        println!("{} {}",
            style("NOTE:").yellow(),
            style(format!("Hourly cost (${:.2}/hr) is approaching threshold (${}/hr)", 
                summary.total_cost_estimate, hourly_threshold)).yellow()
        );
    }
    ...
    if daily_cost > daily_threshold {
        ...
        println!("{} {}",
            style("WARNING:").red().bold(),
            style(format!("Daily projection (${:.2}/day) exceeds threshold (${}/day)", 
                daily_cost, daily_threshold)).red().bold()
        );
    }
    ...
    if total_accumulated > accumulated_threshold {
        ...
        println!("{} {}",
            style("WARNING:").red().bold(),
            style(format!("Accumulated cost (${:.2}) exceeds threshold (${})", 
                total_accumulated, accumulated_threshold)).red().bold()
        );
...
async fn cleanup_zombies(dry_run: bool, force: bool, _config: &Config) -> Result<()> {
    ...
    for reservation in reservations {
        ...
        for instance in instances {
            ...
            // Check if protected
            let is_protected = instance.tags()
                .iter()
                .any(|t| {
                    t.key().map(|k| k == "trainctl:protected" || k == "trainctl:important" || k == "trainctl:persistent").unwrap_or(false) &&
    ...
    for volume in volumes_response.volumes() {
        ...
        let has_trainctl_tag = volume.tags()
            .iter()
            .any(|t| {
                t.key().map(|k| k.contains("trainctl")).unwrap_or(false)
            });
    ...
    if zombies.is_empty() && orphaned_volumes.is_empty() {
        ...
        if !protected_instances.is_empty() {
            println!("   ({} protected instance(s) skipped)", protected_instances.len());
        }
        return Ok(());
    }
    ...
    if !orphaned_volumes.is_empty() {
        println!("\nFound {} orphaned volume(s) (available, not persistent):", orphaned_volumes.len());
    ...
    if !protected_instances.is_empty() {
        println!("\nSkipped {} protected instance(s):", protected_instances.len());
...
async fn show_insights(_config: &Config, output_format: &str) -> Result<()> {
    ...
    if old_instances > 0 {
        println!("WARNING: {} instance(s) running > 24 hours - consider terminating", old_instances);
    }
    ...
    if total_cost > 10.0 {
        println!("  WARNING: High hourly cost (${:.2}/hr) - review instance types", total_cost);
    }
...
async fn display_table_format(instances: &[&InstanceInfo], detailed: bool) -> Result<()> {
    ...
    if detailed {
        ...
        for inst in instances {
            ...
            let name_tag = inst.tags.iter()
                .find(|(k, _)| k == "Name")
                .map(|(_, v)| v.as_str())
                .unwrap_or("-");
            let other_tags: String = inst.tags.iter()
                .filter(|(k, _)| k != "Name")
                .take(1)
                .map(|(k, v)| format!("{}={}", k, v))
                .collect::<Vec<_>>()
...
"#;
        ...
        let _ = crate::aws_utils::execute_ssm_command(&ssm_client, instance_id, graceful_stop_cmd).await;
